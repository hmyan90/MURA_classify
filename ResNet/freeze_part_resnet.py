...
def resNet50_model(img_rows, img_cols, channel=1, num_classes=None):
    ...
    base_resNet = ResNet50(weights='imagenet', include_top=False, input_shape=(img_rows, img_cols, channel))
    
    x = base_resNet.output # (1, 1, 2048)
    
#     x = Flatten()(x) # 2048
    x = GlobalAveragePooling2D()(x) # 2048
    
    # add fully-connected & dropout layers
    x = Dense(1000, activation='relu',name='fc-1', kernel_initializer='uniform')(x)
    x = Dropout(0.3)(x)
    
    # add fully-connected & dropout layers
    x = Dense(500, activation='sigmoid',name='fc-2')(x)
    x = Dropout(0.4)(x)
    
    # add another fully-connected & dropout layers
    x = Dense(100, activation='sigmoid',name='fc-3')(x)
    x = Dropout(0.2)(x)
    ...
    transfer_resNet = Model(inputs=base_resNet.input, outputs=x)
    
#     for layer in transfer_resNet.layers[:-150]:
...
#     sgd = SGD(lr=1e-3, decay=1e-6, momentum=0.9, nesterov=True)
    ...
    transfer_resNet.compile(optimizer=adam, loss=binary_crossentropy, metrics=[binary_accuracy])

    return transfer_resNet
...
checkpoint = ModelCheckpoint(filepath='ResNet/model2_weight.hdf5', verbose=1, save_best_only=True)
early_stop = EarlyStopping(patience=10)
now_iso = datetime.now().strftime('%Y-%m-%dT%H:%M:%S%z')
tensorboard = TensorBoard(log_dir='ResNet/logs/{}/'.format(now_iso))
callbacks = [checkpoint, tensorboard, early_stop]
    
print("Saved model to disk")
model_json = model.to_json()
with open("ResNet/model2.json", "w") as json_file:
